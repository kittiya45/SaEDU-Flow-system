...
// แยกเลขนำหน้าชื่อฟอร์ม เช่น "1.1 แบบฟอร์ม..." -> [1,1] เพื่อเรียงแบบ natural sort
function _tmplNumParts(name){
  var m=(name||'').trim().match(/^(\d+(?:\.\d+)*)/);
  if(!m) return null;
  return m[1].split('.').map(Number);
}
function _tmplCompare(a,b){
  var pa=_tmplNumParts(a.name),pb=_tmplNumParts(b.name);
  if(pa&&pb){
    var len=Math.max(pa.length,pb.length);
    for(var i=0;i<len;i++){
      var va=pa[i]!==undefined?pa[i]:-1;
      var vb=pb[i]!==undefined?pb[i]:-1;
...
async function vTmpl(){
  ...
  if(rows.length){
    ...
    if(_uUniq.length){
      try{
        var _uRows=await dg('user_directory','?id=in.('+_uUniq.map(safeId).join(',')+')'+'&select=id,full_name');
        if(Array.isArray(_uRows)) _uRows.forEach(function(u){_uploaderMap[u.id]=u.full_name});
      }catch(e){}
    }
    // ไฟล์ที่อัปโหลดล่าสุดในระบบ (ไม่ว่าจะเป็นฟอร์มชื่อใด) — แสดงแก่ทุกคน
    var _latest=rows.reduce(function(a,b){return(a.created_at||'')>=(b.created_at||'')?a:b});
    _latestIds[_latest.id]=true;
  }
  ...
  Object.keys(TMPL_CATS).forEach(function(cat){
    ...
    items.forEach(function(t,i){
      ...
      if(_latestIds[t.id]) html.push('<span class="badge b-completed" style="font-size:9px;padding:2px 7px;flex-shrink:0">ล่าสุด</span>');
      html.push('</div>');
...
function showTmplEdit(id, name, cat, desc){
  var w = $e('mwrap');
  if(!w) return;

  var catOpts = Object.entries(TMPL_CATS).map(function(e){
    return '<option value="'+e[0]+'"'+(e[0]===cat?' selected':'')+'>'+e[1]+'</option>';
  }).join('');

  w.innerHTML = [
    '<div id="tmpl-edit-overlay" class="mo" style="position:fixed; inset:0; z-index:9999; background:rgba(24,18,14,0.4); backdrop-filter:blur(8px); display:flex; align-items:center; justify-content:center;">',
      '<div class="modal gnk-box" style="max-width:520px; width:95%; border-radius:28px; background:#fff; box-shadow:0 40px 100px -20px rgba(232,58,0,.15); border:1px solid rgba(232,58,0,.05); overflow:hidden; position:relative;" onclick="event.stopPropagation()">',
        '<div class="modal-head" style="padding:28px 32px 20px 32px; background:linear-gradient(to bottom, rgba(232,58,0,.05), #fff); border:none; display:flex; justify-content:space-between; align-items:flex-start;">',
          '<div class="flex flex-col gap-1">',
            '<div style="color:#E83A00; font-weight:800; font-size:10px; letter-spacing:.12em; text-transform:uppercase; display:flex; align-items:center; gap:6px;">',
            ...
            '</span>',
          '</div>',
          '<button type="button" id="btn-close-tmpl-edit" class="btn btn-soft sm btn-icon" style="cursor:pointer; background:rgba(232,58,0,.08); color:#E83A00; border-radius:12px; width:34px; height:34px; border:none; display:flex; align-items:center; justify-content:center;">',
            svg('x',14),
          '</button>',
        '</div>',
        '<div class="modal-body" style="padding:0 32px 24px 32px;">',
          '<div class="fg" style="margin-bottom:16px;">',
            '<label class="fl" style="display:block; font-size:11.5px; font-weight:700; color:#18120E; margin-bottom:6px;">',
              'ชื่อแบบฟอร์ม <span class="req" style="color:#E83A00">*</span>',
            '</label>',
            '<input class="fi" id="tmpl-edit-name" value="'+esc(name||'')+'" style="width:100%; height:44px; padding:0 14px; font-size:12.5px; border-radius:12px; border:1.5px solid #F1F1F1; background:#FAFAFA; outline:none;">',
          '</div>',
          '<div class="fg" style="margin-bottom:16px;">',
            '<label class="fl" style="display:block; font-size:11.5px; font-weight:700; color:#18120E; margin-bottom:6px;">',
              'หมวดหมู่ <span class="req" style="color:#E83A00">*</span>',
            '</label>',
            '<select class="fi" id="tmpl-edit-cat" style="width:100%; height:44px; padding:0 10px; font-size:12.5px; border-radius:12px; border:1.5px solid #F1F1F1; background:#FAFAFA;">',
              catOpts,
            '</select>',
          '</div>',
          '<div class="fg">',
            ...
            '</label>',
            '<input class="fi" id="tmpl-edit-desc" value="'+esc(desc||'')+'" placeholder="เช่น ไม่ต่ำกว่า 3 สัปดาห์ก่อนเริ่มกิจกรรม/โครงการ" style="width:100%; height:44px; padding:0 14px; font-size:12.5px; border-radius:12px; border:1.5px solid #F1F1F1; background:#FAFAFA; outline:none;">',
          '</div>',
        '</div>',
        '<div class="modal-foot" style="padding:0 32px 32px 32px; border:none; background:none; display:flex; gap:10px;">',
          '<button type="button" id="btn-cancel-tmpl-edit" class="btn btn-soft" style="cursor:pointer; flex:1; height:46px; border-radius:14px; font-weight:600; font-size:12.5px; border:1px solid #EBEBEB; background:#fff; color:#6b6560;">',
            'ยกเลิก',
          '</button>',
          '<button type="button" id="btn-save-tmpl-edit" class="btn btn-primary" style="cursor:pointer; flex:1.8; height:46px; border-radius:14px; font-weight:700; font-size:12.5px; background:#E83A00; color:#fff; display:flex; align-items:center; justify-content:center; gap:8px; border:none; box-shadow:0 8px 16px -4px rgba(232,58,0,.3);">',
            'บันทึก',
          '</button>',
        '</div>',
      '</div>',
  ...
  var overlay=document.getElementById('tmpl-edit-overlay');
  var closeBtn=document.getElementById('btn-close-tmpl-edit');
  var cancelBtn=document.getElementById('btn-cancel-tmpl-edit');
  var saveBtn=document.getElementById('btn-save-tmpl-edit');
  ...
  saveBtn.addEventListener('click', function(e){e.preventDefault();e.stopPropagation();doTmplEdit(id);});
}

async function doTmplEdit(id){
  var name=(gv('tmpl-edit-name')||'').trim();
  var cat=gv('tmpl-edit-cat')||'general';
  var desc=(gv('tmpl-edit-desc')||'').trim();
  if(!name){alert('กรุณาระบุชื่อแบบฟอร์ม');return}
  var btn=$e('btn-save-tmpl-edit');
  if(btn){btn.disabled=true;btn.innerHTML='<span class="sp"></span> กำลังบันทึก...'}
  try{
    await dpa('form_templates',id,{name:name,category:cat,description:desc||null});
    $e('mwrap').innerHTML='';
    var a=$e('tal'); if(a) a.innerHTML=alrtH('ok','บันทึกการแก้ไขเรียบร้อยแล้ว');
