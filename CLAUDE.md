...
**`dashboard.js` is dead code** — it still defines `vDash()`/`vTodo()`, but `index.html` does not load it; `homeViews.js` is the live copy. Edit homeViews.js. (The `<script>` was dropped because ad blockers commonly block a file named `dashboard.js`; the file itself was left behind and has since drifted.)
