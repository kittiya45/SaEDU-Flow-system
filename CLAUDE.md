...
- **User expiry check** — after password verification, if `user_type === 'gnk'` and `expires_at` is set and in the past: sets `is_active = false` in DB and rejects login with "บัญชีหมดอายุ" message
...
- `_ACTIVE_STEPS` — `{docId: full_name}` map populated in `vDocs()`; used by `rDocTbl` to show "รอ: [ชื่อ]" under the status badge for pending documents
...
- `GNK-PRE` (หัวหน้านิสิต) is auto-added as a locked mandatory signer at the end of every **incoming** document's workflow only. This is done inside `selectDocType()` in `docForm.js`. The locked step is skipped if the creator is GNK-PRE **or if the creator is `ROLE-STF` / `ROLE-SYS`** (staff bypass — they can number immediately with no workflow). **Outgoing docs do NOT get GNK-PRE added** (no workflow at all).
...
- `ROLE-STF` sees **all documents** in `vDocs()` and `vDash()` (same as `ROLE-SYS` / `GNK-SEC`) — needed so forwarded incoming docs appear in staff's list.
...
The `ตำแหน่ง / สังกัด` dropdown (`_spOpts`, element `#fto`) for **incoming docs** is grouped with `<optgroup>` into three categories: **กนค.** (non-club, code 01–14), **แผนก** (non-club, code 15+), **ชมรม** (`isClub: true`). Filter logic is in `docForm.js` just before `renderTypeFields`.
...
- Approval modal: signature is **mandatory** (not optional); user selects which page to place the signature (default: last page) and can zoom in/out to inspect before clicking to place
...
- **`doc_number` is auto-assigned at creation** via `genOutDocNumber()` in format `กนค.YYYY.NN` (2-digit sequence, e.g. `กนค.2569.01`); sequence resets each Thai year and respects the admin-configured `out_start_seq`
...
**Outgoing:** `draft` → `completed` (submit, instant — no intermediate steps; `doc_number` set at creation)
...
- `GNK_NUM` — GNK position code → 2-digit numeric string (e.g. `'GNK-PRE' → '01'`); used for **incoming** PP digit only
...
- `_resubBusy` — same pattern for `doReSubmit`. Reset via `.catch()` in `events.js` **and also explicitly** on every early-return path inside `doReSubmit` (e.g. `!rejStep` guard sets `_resubBusy=false` before returning).
...
`doAct` final-status logic: `nst = allDone ? (doc.doc_type==='incoming' ? 'numbering' : 'completed') : 'pending'` — only **incoming** transitions to `numbering`; all other types go directly to `completed`.
...
- `exportCSV()` — **async**; batch-fetches creator names (`select=id,full_name`) for all unique `created_by` UUIDs in `ADOCS` before building rows, so the "ผู้สร้าง" column contains full names not UUIDs. UTF-8 BOM CSV for Thai text in Excel.
...
- `S` (หลักที่ 1) — ภาคการศึกษา: `1` = ต้น, `2` = ปลาย (user selects) — **shown first in modal**
- `PP` (หลักที่ 2–3) — `SENDER_POS.find(p => p.name === doc.addressed_to).code` — auto-filled, read-only
...
Modal field order (หลักที่ 1 → 2–3 → 4 → date → font slider → preview → forward).  
**วันที่หนังสือ** is auto-set to today (read-only display `div` + hidden `#num-docdate` input); `doSetDocNumber` reads `gv('num-docdate')`.  
**Font size slider** (`#num-fontsize`, 8–24px) calls `_updateStampFontSize()` to resize the draggable stamp overlays and is read in `doSetDocNumber` as `_stampSz` for `_pg.drawText(...)`.  
**Thai font** (`thsarabunnew-webfont.ttf`) is fetched once and cached in the module-level `_thFontCache` variable; subsequent numbering calls use `_thFontCache.slice(0)` without re-fetching.  
**Forward dropdown** (`#num-fwd`) queries only `ROLE-STF` users — `dg('users','?role_code=eq.ROLE-STF&...')`.  
**PDF.js version**: all lazy-loads (docNum, viewer, editor) now use **v3.11.174** from cdnjs. If `window.pdfjsLib` is already loaded, only the worker URL is set if missing.

`doSetDocNumber` does **not** prepend the doc number to `doc.title`; it is stored only in `doc_number`.

**Outgoing doc numbering** is handled separately in `docForm.js` via `genOutDocNumber()` at creation time (not through `docNum.js`). Format: `กนค.YYYY.NN` (2-digit sequence). The `showNumModal`/`doSetDocNumber` outgoing branch is dead code retained for legacy compatibility.

### Approval modal — signature placement (`docDetail.js`)
The approve modal (`showActModal`) uses a 2-column layout (880px wide): left = signature form, right = PDF viewer with toolbar.

Module-level state reset on each modal open: `_actSigPos={xFrac:null,yFrac:null}`, `_actSigPdf=null`, `_actSigPage=1`, `_actSigZoom=1.0`.

**Toolbar** (above PDF preview, shown after PDF loads):
- **Page navigation** — `◀ หน้า N/T ▶` buttons (`_sigPageNav(dir)`); hidden when PDF has only 1 page; changing page resets `_actSigPos` to default bottom-right
- **Zoom** — `zout / N% / zin` buttons (`_sigZoom(±0.25)`); range 50%–300% in 25% steps; zoom re-renders the canvas at `baseScale × _actSigZoom` without resetting the chosen signature position

`_loadSigPosPreview(docId)` — lazy-loads PDF.js, stores the loaded PDF document in `_actSigPdf`, defaults to last page, calls `_renderSigPage()`.

`_renderSigPage(pageNum, resetPos, wrap, hint)` — renders a single page to canvas at the current zoom level. `resetPos=true` resets position to bottom-right; `resetPos=false` (zoom-only re-render) keeps existing `_actSigPos` fractions.

`_actSigPos` `{xFrac, yFrac}` stores the chosen position as fractions (0–1) of page width/height from top-left. In `doAct`, used to compute pdf-lib coordinates: `pdfX = xFrac*pw`, `pdfY = (1-yFrac)*ph - 60`; page index is `(_actSigPage - 1)` (0-based for pdf-lib).
...
**Recipient selection by status/action:**
- `newStatus === 'completed'` OR `newStatus === 'numbering'` → notify document **creator** (numbering means all signatures done; creator must issue the number)
- `action === 'approve'` and next active step exists → notify **next step's assignee**
- `action === 'reject'` → notify **creator**
- `action === 'create'` or `'resubmit'` → notify **next active step's assignee**
- `action === 'overdue'` → notify both active assignee and creator

**Email status pattern** — `var status='failed'` is declared **before** the `try` block; only assigned inside `try` (`status = resp.ok ? 'sent' : 'failed'`). Never re-declare `var status` in the `catch` block — `var` hoisting makes duplicate declarations in the same function scope a reliability risk.

**Forward notification pattern** (`doForward` in `docDetail.js`, `doSetDocNumber` forward path in `docNum.js`): always write a `notifications` row regardless of email deliverability. Email is attempted only when `recipEmail && !recipEmail.includes('@gnk.student')`; the notification `status` field is `'sent'`, `'failed'`, or `'skipped'` (skipped = no valid external email). This ensures the in-app forward badge and PC count work even for internal-only accounts.
...
Upload/delete functions (`showTmplUpload`, `doTmplUpload`, `doTmplDelete`) are available to `ROLE-SYS`, `ROLE-STF`, **and `GNK-SEC`** — controlled by `isAdm` flag in `vTmpl()`.  
...
### Admin modules
- `admin.js` — `vAdm()` user management view. Both `ROLE-SYS` and `ROLE-STF` have full access.
  - **User expiry**: GNK accounts have an `expires_at` column. `admApv()` sets `expires_at = now + 1 year` when approving a GNK user. `admRenew(uid)` extends by another year and re-activates the account. Expired users show a red "หมดอายุ" badge with expiry date.
  - **User deletion** (`admDel`): separates documents into `dcCreated` (created by the user — **deleted**) and `dcRef` (only forwarded/final-recipient references — **unlinked**). Workflow steps assigned to the deleted user are unlinked (`assigned_to = null`).
  - The subtitle under each user's name displays `PTH[position_code]` for GNK members; staff shows "เจ้าหน้าที่กิจการนิสิต", advisors show "อาจารย์กิจการ". The position column shows Thai name + raw code on two lines.
- `adminImport.js` — CSV import guarded by `ROLE-SYS || ROLE-STF`
- `sysAdmin.js` — `vSys()` system settings (ROLE-SYS only). Manages the `doc_number_settings` table which controls:
  - **Incoming prefix** — `prefix` column (e.g. `GNK`), used in `genDocNumber()` to build `GNK-YYYY-NNN`
  - **Outgoing start sequence** — `out_start_seq INTEGER` column; sets the minimum sequence number for outgoing docs in a given year, used by `genOutDocNumber()` in `docForm.js`
  - The settings card shows both incoming and outgoing format previews side by side
...
Document visibility in `vDocs()`:
- `ROLE-SYS`, `ROLE-STF`, `GNK-SEC` → all documents
- Others → documents they created OR have a workflow step in OR **`forwarded_to_id === CU.id`**

**Active step display**: after loading `ADOCS`, `vDocs()` batch-fetches active workflow steps for all pending documents, then resolves `assigned_to` user IDs to full names. The result is stored in `_ACTIVE_STEPS {docId: full_name}`. `rDocTbl` renders "รอ: [ชื่อ]" in small text under the status badge for pending documents. This requires 2 additional queries but only when there are pending docs.

The `forwarded_to_id` condition is added to the Supabase OR query so recipients of forwarded documents see the doc in their list without needing admin access. `rDocTbl` renders a **📨 ส่งถึงคุณ** badge (`.badge.b-blue`) on each row where `d.forwarded_to_id === CU.id`.

`setDT(tab)` resets `_PROJ_FILTER=''` and clears the `#dproj` dropdown value when switching tabs — prevents outgoing project filter leaking into non-outgoing tabs. The `#dproj` dropdown is hidden (`display:none`) when there are no outgoing documents in `ADOCS`.

**Tab badge colours** — `_redTabs = ['mine', 'rejected', 'numbering', 'fwd']`. The "รอลงนาม" (`pending`) tab uses a neutral grey badge — it is informational only and does not trigger red alert styling.
...
- `svg(name, size)` — returns inline SVG by icon name (primary icon system; Lucide CDN is loaded but rarely used). Available icons include: `home`, `doc`, `plus`, `sign`, `up`/`dn`, `edit`, `trash`, `ok`, `x`, `back`, `eye`, `srch`, `out`, `save`, `bell`, `undo`, `zin`, `zout`, `lock`/`unlock`, `key`, `shield`, `cal`, `chart`, `warn`, `info`, `folder`, `refresh`, `pen`, `dots`, `gear`, `clock`, `tri`, `tasks`, and filled variants via `svgf()`
...
- `PC` (pending doc badge on เอกสาร menu) counts: pending/rejected docs the user owns or has workflow steps in **plus** all documents where `forwarded_to_id = CU.id`. For `ROLE-SYS` it counts all pending/rejected system-wide.
- All `<script>` tags in `index.html` use `defer` — scripts download in parallel and execute in document order after DOM is parsed. The inline `tailwind.config` block runs synchronously before Tailwind CDN, which is correct.
- `vDash()` fetches documents with `limit=500&select=id,title,doc_type,status,urgency,due_date,created_by,created_at` to cap payload size; dashboard stat cards only need these columns.
- `<link rel="preconnect">` hints for Supabase, cdnjs, and jsdelivr are in `<head>` to warm DNS/TLS before first fetch.
- Login queries run as `Promise.all([student_id lookup, email lookup])` — two parallel Supabase calls instead of sequential.
- Dashboard `vDash()` fetches `documents` and `workflow_steps` in parallel via `Promise.all`.
- Lazy CDN libraries (PDF.js v3.11.174, pdf-lib, fontkit, mammoth) are loaded on first use via `loadSc()`. **Do not revert to synchronous `<script>` tags for these.**
- Thai font for PDF stamping is cached in `_thFontCache` (module-level in `docNum.js`) to avoid re-fetching on every numbering action.

## DB schema notes (columns added after initial deploy)

These columns were added via `ALTER TABLE` and may not appear in Supabase's auto-generated type definitions:

```sql
-- ตั้งค่าเลขหนังสือขาออก (สำหรับ genOutDocNumber ใน docForm.js)
ALTER TABLE doc_number_settings ADD COLUMN IF NOT EXISTS out_start_seq INTEGER DEFAULT 1;

-- อายุบัญชีนิสิต (ตรวจสอบใน doLogin, ตั้งค่าใน admApv/admRenew)
